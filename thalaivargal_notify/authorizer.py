...
# Invoke:  sam local invoke --event events/authorize_event.json QuotesAuthFunction
...
def handler(event, context):
    ...
    '''
    principalId = 'NotifyQuotesAppUser'
    ...
    '''
    key = "487f4e317959fc703a278d1b2e066d33e3b0aa41fac164aa52aa3281b6ca3b60"

    encoded = event['authorizationToken']
    # decoded = jwt.decode(encoded, key, audience='tamil-quotes', algorithms=['HS512'])
    ...
    policy.allowMethod(HttpVerb.GET, '/notify')
    policy.allowMethod(HttpVerb.POST, '/notify')
    policy.allowMethod(HttpVerb.GET, '/quotes-api/*')
    policy.allowMethod(HttpVerb.POST, '/quotes-api/*')
    policy.allowMethod(HttpVerb.PUT, '/quotes-api/*')
    policy.allowMethod(HttpVerb.PATCH, '/quotes-api/*')
    policy.allowMethod(HttpVerb.DELETE, '/quotes-api/*')
